...
def read_beam_file(fileName):
    ...
    beam = GenesisBeamDefinition()
    if fileName is None: #HMCC (correcting bug filename None)
        fileName= ''
    f=open(fileName,'r')   
    null=f.readline()
    for line in f: 
...
#
    ...
    if out('dgrid')==0:
        ...
        ray=sqrt((out('zrayl')*out('xlamds')/np.pi)*(1+(out('zwaist')/out('zrayl'))**2));#bug fixed HMCC
        out.leng=out('rmax0')*(rbeam+ray)#bug fixed HMCC
        #rbeam=sqrt(out('rxbeam')**2+out('rybeam')**2) Original implementation
        #ray=sqrt(out('zrayl')*out('xlamds')/np.pi*(1+(out('zwaist')/out('zrayl')))**2); Original implementation
