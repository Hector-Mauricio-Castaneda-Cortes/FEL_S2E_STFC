...
class MpiLauncher(Launcher):
    ...
    def launch(self):
        ...
        ##### HMCC #####
        if self.program == "genesis2.0":
            command_genesis = 'echo'+ " "+self.filename + " |mpiexec -np 25" + str(self.mpiParameters) + " " + self.program
        elif self.program == "genesis3.2.1":
            command_genesis = "mpirun -np 25" + str(self.mpiParameters) +" " +self.program + " " + self.filename
        command = 'mkdir -p '+ self.dir + '; ' + 'cd '+ self.dir + '; ' + command_genesis
        #command = 'mkdir -p '+ self.dir + '; ' + 'cd '+ self.dir + '; '+ 'echo'+ " "+self.filename + " |mpiexec -np 25" + str(self.mpiParameters) + " " + self.program #HMCC
        ######################
        print command
        os.system(command)
        t2 = time.time()
