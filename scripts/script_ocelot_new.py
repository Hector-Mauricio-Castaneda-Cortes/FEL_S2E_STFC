...
def beta_matching(inp,f_path):
    ...
    with open(f_path+'/beta_input_file.in','w') as f:
        f.write('mod_file.in')
    f.close()
    with open(f_path+'/lattice.inp','w') as f:
        f.write(inp0.lattice_str)
    f.close()
    try:
...
def gen_outplot_single(proj_dir,run_inp= [], itdp = True,savefig=True):
    import matplotlib.pyplot as plt
    from ocelot.gui.genesis_plot import subfig_rad_pow
    from ocelot.adaptors.genesis import readGenesisOutput
    import copy, os
    ...
    for i_run,runr in enumerate(run_range):
        ...
        for i_seed, ipseed in enumerate(ip_seed):
            ...
            if os.path.isfile(out_file[0]):
                g=readGenesisOutput(str(out_file[0]),readall=1)
                ip_s.append(g)
...
# Setting off LaTeX in order to avoid the error when a plot is generated.
    ...
    if (A_input['i_scan']==0):
        if (A_input['stat_run'] >1):
            ...
            fig=gen_stat_plot(exp_dir,run_inp=s_scan,param_inp=['p_int','energy','el_e_spread','bunching','spec','xrms','yrms'],stage_inp=xrange(1),dfl_param_inp=[],s_inp=['max'],z_inp=['end'], savefig=1, saveval=1, showfig=0)
